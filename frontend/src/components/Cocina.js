...
const Cocina = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    const cargarCat = () => {
      ...
      fetch(`${apiUrl}/ingredientes?t=${t}`)
        .then(res => res.json())
        .then(data => { if (Array.isArray(data)) setCatalogoIngredientes(data); })
        .catch(() => {});
        
      fetch(`${apiUrl}/clasificaciones?t=${t}`)
        .then(res => res.json())
        .then(data => { if (Array.isArray(data)) setClasificaciones(data); })
        .catch(() => {});
    };
    cargarCat();
  ...
  const fetchPedidos = useCallback(async () => {
    try {
      ...
      data = data.filter(p => ['Pagado', 'Preparando', 'Por Confirmar'].includes(p.estado_preparacion));
  ...
  const actualizarEstadoPedido = async (pedido, nuevoEstado, trabajadorId) => {
    ...
    try {
      await fetch(`${apiUrl}/pedidos/${pedido.id}/estado`, {
        ...
        body: JSON.stringify({ 
          ...
          chef_id: trabajadorId || pedido.chef_id 
        })
      });
      fetchPedidos();
  ...
  const enviarAlerta = async () => {
    ...
    setIsSubmitting(true);
    try {
      const msj = `[IDX:${modalAlerta.item.indices.join(',')}] 🚨 FALTANTE: ${faltanteSelec}. PROPUESTA COCINA: ${propuestaSelec}`;
  ...
  const obtenerOrdenActivaDeTrabajador = (id) => pedidos.find(p => String(p.chef_id) === String(id) && p.estado_preparacion === 'Preparando');
  const obtenerNombreTrabajadorActivo = () => user?.nombre || 'Chef';
    ...
    <div className="min-h-screen flex flex-col font-sans bg-slate-950 transition-colors duration-500 overflow-hidden relative">
      ...
      <main className="flex-1 p-4 md:px-8 md:pb-8 overflow-y-auto custom-scrollbar relative">
        <GridPedidos
          ...
          obtenerOrdenActivaDeTrabajador={obtenerOrdenActivaDeTrabajador}
      ...
      {modalMerma && (
        <ModalReportarMerma 
          setModalMerma={setModalMerma} 
          catalogoIngredientes={catalogoIngredientes} 
          apiUrl={apiUrl} refrescarDatos={fetchPedidos} 
        />
      )}
      ...
      {/* 👇 AQUÍ LE PASAMOS LA FUNCIÓN PARA QUE AL ACEPTAR EL PIN, ACTUALICE AL AYUDANTE DE INMEDIATO */}
      <ModalAsistencia 
