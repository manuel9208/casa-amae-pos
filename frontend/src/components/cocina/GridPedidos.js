  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {pedidosVisibles.map(p => {
        ...
        if (itemsDeEstaArea.length === 0) return null;
        ...
        // 👇 LÓGICA DE ASIGNACIÓN Y SEGURIDAD
        const idAAsignar = ayudanteSeleccionado || trabajadorActivoId;
        const ordenPendienteDelActivo = obtenerOrdenActivaDeTrabajador(idAAsignar);
        const trabajadorEstaOcupado = !!ordenPendienteDelActivo;
        
        // Averiguar si el empleado que está seleccionado arriba es el dueño legítimo de este pedido
        const soyElDueño = String(p.chef_id) === String(idAAsignar);

        // Buscar el nombre del dueño para mostrarlo en el botón bloqueado
        let nombreDueño = 'Otro ayudante';
        if (trabajadoresActivos && trabajadoresActivos.length > 0) {
           const dueñoTicket = trabajadoresActivos.find(t => String(t.id) === String(p.chef_id));
           if (dueñoTicket) nombreDueño = dueñoTicket.nombre;
        }

        let nombreDelActivo = obtenerNombreTrabajadorActivo();
        if (trabajadoresActivos && trabajadoresActivos.length > 0) {
           const tAct = trabajadoresActivos.find(t => String(t.id) === String(idAAsignar));
           if (tAct) nombreDelActivo = tAct.nombre;
        }
          ...
          <div key={p.id} className={`bg-slate-800 rounded-[30px] p-6 border-2 flex flex-col transition-all duration-300 ${colorBorde} ${shadow}`}>
            ...
            <div className="pt-6 mt-4 border-t border-slate-700">
              {areaEstado === 'Pagado' || !areaEstado || areaEstado === 'Por Confirmar' ? (
                <button
                  onClick={() => actualizarEstadoPedido(p, 'Preparando', idAAsignar)}
                  ...
                  className={`w-full py-4 rounded-2xl font-black text-sm transition duration-200 flex flex-col items-center justify-center gap-0.5 ${
                    trabajadorEstaOcupado
                      ...
                      : 'bg-emerald-500 hover:bg-emerald-400 text-white shadow-[0_0_15px_rgba(16,185,129,0.4)] active:scale-95'
                  }`}
                >
                  <div className="flex items-center gap-2 text-base">
                    {trabajadorEstaOcupado ? <Lock size={16}/> : <ChefHat size={18}/>}
                    <span>{trabajadorEstaOcupado ? 'Ocupado actualmente' : '¡Preparar Orden!'}</span>
                  </div>
                  {trabajadorEstaOcupado && (
                ...
                // 👇 SOLUCIÓN 2: Candado de seguridad (Solo el dueño puede hacer clic)
                <button
                  disabled={!soyElDueño || !!p.alerta_cocina || isSubmitting}
                  onClick={() => actualizarEstadoPedido(p, 'Listo', idAAsignar)}
                ...
                >
                  {soyElDueño ? <CheckCircle2 size={20}/> : <Lock size={20}/>}
                  <span>{soyElDueño ? '¡Terminar Pedido!' : `Preparando: ${nombreDueño}`}</span>
                </button>
              ) : null}
            </div>
          </div>
