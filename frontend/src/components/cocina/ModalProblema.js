...
const ModalProblema = ({
  ...
  setPropuestaSelec, catalogoIngredientes, clasificaciones, isSubmitting, enviarAlerta
}) => {
  return (
          ...
          <div className="bg-slate-900 p-6 rounded-3xl border border-slate-700">
            ...
            <div className="flex flex-wrap gap-2">
              {(() => {
                ...
                const extrasAgregados = (modalAlerta.item.extras || [])
                  .filter(e => !e.nombre.startsWith('Sin ') && !e.nombre.startsWith('📝') && !e.nombre.startsWith('🔸'))
                  .map(e => ({ nombre: e.nombre }));

                const opcionesBrutas = [
                  { id: 'platillo_completo', nombre: `Todo el platillo (${modalAlerta.item.nombre})` }, 
                  ...basesDelPlatillo, 
                  ...extrasAgregados
                ];  

                const opcionesUnicas = Array.from(new Map(opcionesBrutas.map(item => [item.nombre, item])).values());

                return opcionesUnicas.map((b, idx) => (
            ...
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-700 animate-in fade-in">
              ...
              <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto custom-scrollbar pr-2">
                {faltanteSelec.startsWith('Todo el platillo') ? (
                  ...
                  <>
                    {(() => {
                      const categoriaItem = String(modalAlerta.item.categoria || modalAlerta.item.clasificacion_nombre || '').trim().toLowerCase();
                      
                      // 👇 FIX MÁGICO: Buscamos el ID de la clasificación cruzando tablas
                      ...
                      // Ahora filtramos el catálogo por ID o por Nombre (Lo que sea que encuentre primero)
                      const opcionesCatalogo = catalogoIngredientes.filter(i => {
                        const matchId = clasifId && Number(i.clasificacion_id) === Number(clasifId);
                        const matchNombre = String(i.clasificacion_nombre || '').trim().toLowerCase() === categoriaItem;
                        return matchId || matchNombre;
                      });
                      
                      const opcionesPlatillo = modalAlerta.item.opciones || [];  
                      const extrasPlatillo = modalAlerta.item.extras_disponibles || modalAlerta.item.extrasDisponibles || [];
                      ...
                      extrasPlatillo.forEach(o => propuestasMap.set(o.nombre, o.nombre));
                      
                      propuestasMap.delete(faltanteSelec);  
                      const propuestas = Array.from(propuestasMap.values());  

                      if (propuestas.length === 0) {
                          return <p className="text-slate-500 text-sm italic w-full">No hay otros ingredientes registrados en esta categoría.</p>;
                      }
