...
const ModalSolicitarInsumo = ({ setModalInsumo, catalogoIngredientes, apiUrl, user }) => {
  ...
  const enviarSolicitud = async (e) => {
     ...
     try {
       ...
       // Inyectamos como un log o alerta fantasma a pedidos de hoy para alertar a Caja
       await fetch(`${apiUrl}/pedidos/999999/alerta`, { // Usamos ID alto simulado o endpoint log
      ...
      <form onSubmit={enviarSolicitud} className="bg-slate-800 p-8 rounded-[40px] shadow-2xl w-full max-w-md border border-slate-700 text-left">
         ...
         <div className="space-y-4 mb-6">
            ...
            <div>
               ...
               <input required type="text" placeholder="Ej: 2 Kilos, 3 Cajas, 5 Piezas" value={cantidad} onChange={e=>setCantidad(e.target.value)} className="w-full p-4 bg-slate-900 border border-slate-700 rounded-xl outline-none text-white font-bold" />
            </div>
         </div>
         
         <div className="flex gap-4">
            <button type="button" onClick={() => setModalInsumo(false)} className="flex-1 py-4 bg-slate-700 text-white font-black rounded-2xl hover:bg-slate-600 transition">Cancelar</button>
            <button type="submit" disabled={guardando} className="flex-1 py-4 bg-blue-600 text-white font-black rounded-2xl hover:bg-blue-500 shadow-lg shadow-blue-500/20 transition">Enviar Pedido</button>
         </div>
      </form>
    </div>
  );
