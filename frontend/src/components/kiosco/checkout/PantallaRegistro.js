...
const PantallaRegistro = ({
  ...
  // 👇 NUEVO FIX: Estado para controlar la Alerta Gigante de Costo de Envío
  const [mostrarAlertaEnvio, setMostrarAlertaEnvio] = useState(false);
  ...
  const registrarCliente = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // 👇 Mostrar alerta si es a domicilio antes de enviar a DB
        if (tipoConsumo === 'Domicilio') {
            setMostrarAlertaEnvio(true);
        } else {
  ...
  // 👇 NUEVA FUNCIÓN: Interceptar el botón "Confirmar Celular"
  ...
  // 👇 MODAL DE ALERTA GIGANTE
  const modalAlertaEnvio = mostrarAlertaEnvio && (
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center z-[500] p-4 animate-in fade-in text-left">
        <div className="bg-white p-8 rounded-[40px] shadow-2xl max-w-sm w-full text-center border-4 border-purple-500 animate-in zoom-in-95">
            ...
            <h2 className="text-3xl font-black text-slate-800 mb-4 tracking-tight">¡Aviso Importante!</h2>
            <p className="text-slate-600 font-bold text-lg mb-8 leading-relaxed">
                A tu ticket le hace falta agregar el <strong className="text-purple-600">costo de envío</strong>, el cual será calculado y sumado por caja al confirmar tu orden.
            </p>
            <div className="flex flex-col gap-3">
                <button 
                    onClick={() => seleccionarPago('Pendiente', null, tipoConsumo)} 
                    className="w-full bg-purple-600 text-white py-4 rounded-2xl font-black text-xl shadow-lg shadow-purple-500/30 hover:bg-purple-700 transition active:scale-95"
                >
                    Entendido, Enviar Orden
                </button>
                <button 
                    onClick={() => setMostrarAlertaEnvio(false)} 
  ...
  if (pasoTelefono) {
    if (registroInvitadoActivo) {
      return (
        <div className="max-w-md mx-auto mt-10 text-center animate-in slide-in-from-right relative">
          {modalAlertaEnvio}
    ...
    return (
      <div className="max-w-md mx-auto mt-10 text-center animate-in slide-in-from-bottom-4 relative">
        {modalAlertaEnvio}
