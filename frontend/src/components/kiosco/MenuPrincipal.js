...
const MenuPrincipal = ({
  ...
  // 👇 ACTUALIZADO PARA RESPETAR LOS COMBOS
  const editarItem = (item) => {
    ...
    // Si el usuario quiere editar un combo, lo reiniciamos desde cero conservando su estado
    if (item._esCombo) {
  ...
  // 👇 ACTUALIZADO: REGLA DE INTERCEPCIÓN ENCADENADA
  const abrirModalProducto = (p) => {
    ...
    if (comboMatch) {
        // En lugar de ir al ModalArmarCombo directo, lo mandamos al Personalizador
        setProductoEnEspera({
            ...p,
            _esComboBuilder: true,
            _configuracionCombo: comboMatch
