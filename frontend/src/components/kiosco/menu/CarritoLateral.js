...
const CarritoLateral = ({
  ...
  return (
    <div className="w-full lg:w-1/3 bg-white rounded-3xl md:rounded-[40px] shadow-xl p-5 md:p-8 border flex flex-col h-[60vh] lg:h-full shrink-0 relative animate-in slide-in-from-right duration-300">
      ...
      {pedidoEditandoId && (
        <div className="absolute top-0 left-0 right-0 bg-orange-500 text-white text-center py-1 md:py-2 rounded-t-3xl md:rounded-t-[40px] text-[10px] md:text-xs font-black uppercase tracking-widest shadow-md">
          Editando Orden Activa
        </div>
      )}  

      <h2 className={`text-xl md:text-2xl font-black mb-4 md:mb-6 border-b pb-3 md:pb-4 text-slate-800 shrink-0 ${pedidoEditandoId ? 'mt-4' : ''}`}>
        Tu Orden
      </h2>  
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
        {carrito.length === 0 ? (
          <div className="text-center py-10 md:py-20 opacity-20">
            ...
            <p className="font-bold text-sm md:text-base">Tu carrito está vacío</p>
          </div>
        ) : (
          carrito.map((item) => (
            <div key={item.idTicket} className="flex justify-between items-start mb-3 md:mb-4 bg-slate-50 p-3 md:p-4 rounded-2xl border border-slate-100">
              <div className="flex-1 pr-2 text-left">
                ...
                <ul className="text-[10px] md:text-xs mt-1 space-y-1">
                  {item.extras?.filter(e => !String(e.nombre).includes('🔗 Ref:')).map((e, idx) => (
                    <li key={idx} className={e.nombre.startsWith('Sin ') ? 'text-red-400 line-through font-medium' : e.nombre.startsWith('📝') || e.nombre.startsWith('🔸') || e.nombre.startsWith('📦') ? 'text-slate-600 font-bold bg-white px-2 py-1 rounded-lg border inline-block mt-1' : 'text-blue-500 font-bold'}>
                      {e.nombre}
                    </li>
                  ))}
                </ul>
                <span className="font-black text-blue-600 block mt-2 text-lg md:text-xl">
              ...
              <div className="flex flex-col items-end gap-2 shrink-0">
                ...
                {(!item._esPromo && !item._esSubItemCombo && !item._isCustomizedChild && !item._esCombo && !item._comboGroupId) ? (
                    ...
                    <div className="flex items-center bg-slate-100 border border-slate-200 rounded-xl px-3 py-1 shadow-inner">
                      <span className="font-black text-slate-400 text-[10px] md:text-xs uppercase tracking-widest">Fijo: {item.cantidad || 1}</span>
                    </div>
                )}

                <div className="flex gap-1 md:gap-2 mt-1">
                  {(!item._esSubItemCombo && !item._isCustomizedChild) && (
                    <>
                      ...
                      {/* 👇 FIX: Borrado en Cascada. Si se borra el Padre, se borran sus hijos automáticamente */}
                      <button onClick={() => {
                          quitarDelCarrito(item.idTicket);
                          if (item._comboGroupId) {
                              carrito.forEach(child => {
                                  if (child._comboGroupId === item._comboGroupId && child.idTicket !== item.idTicket) {
                                      quitarDelCarrito(child.idTicket);
                                  }
                              });
                          }
                      }} className="p-1.5 md:p-2 text-red-500 hover:bg-red-100 rounded-xl bg-white shadow-sm border border-slate-100 text-sm">❌</button>
                    </>
                  )}
                </div>
              </div>
