...
import { Search, PlusCircle, Edit2, Trash2, MapPin, Key, Award } from 'lucide-react';  
...
const DirectorioClientes = ({ clientes, formaterMoneda, apiUrl, refrescarDatos, showAlert }) => {
  ...
  // 🛡️ CORRECCIÓN: Agregamos "puntos" al estado inicial para control de fidelidad
  const [formData, setFormData] = useState({
    nombre: '', apellido: '', telefono: '', correo: '', fecha_nacimiento: '', direccion: '', nip: '', puntos: 0
  });  
  ...
  const handleOpenCrear = () => {
    ...
    setFormData({ nombre: '', apellido: '', telefono: '', correo: '', fecha_nacimiento: '', direccion: '', nip: '', puntos: 0 });
  ...
  const handleOpenEditar = (c) => {
    ...
    setFormData({
      ...
      telefono: c.telefono || '',
      correo: c.correo || '',
      // 👇 FIX: Forzamos el formato AAAA-MM-DD aislando la fecha del timestamp ISO
      fecha_nacimiento: c.fecha_nacimiento ? String(c.fecha_nacimiento).split('T')[0] : '',
      direccion: c.direccion || '',
      nip: c.nip || '',
      puntos: c.puntos || 0 // 👇 FIX: Cargamos los puntos actuales para editarlos
    });
    setModalCliente(true);
  };  
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        refrescarDatos(); // Llama a la actualización en vivo de la tabla
      } else {
        ...
        <table className="w-full text-left border-collapse min-w-max">
          <thead>
            <tr className="border-b border-slate-100 text-slate-400 text-[10px] font-black uppercase tracking-widest">
              ...
              <th className="pb-3 px-2">Nacimiento</th>
              ...
              clientesFiltrados.map(c => (
                <tr key={c.id} className="hover:bg-slate-50/50 transition">
                  ...
                  <td className="py-3 px-2">
                    <p className="font-black text-slate-700">{c.nombre} {c.apellido}</p>
                    <div className="flex items-center mt-1">
                      <span className="text-[10px] font-black text-indigo-600 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-md uppercase tracking-widest flex items-center gap-1 w-fit shadow-sm">
                        <Key size={10} /> NIP: {c.nip || '0000'}
                      </span>
                    </div>
                  </td>
                  <td className="py-3 px-2">
                  ...
                  {/* 👇 FIX APLICADO: Renderizamos la fecha estrictamente en AAAA-MM-DD */}
                  <td className="py-3 px-2 text-xs font-black text-slate-500 tracking-wider">
                    {c.fecha_nacimiento ? String(c.fecha_nacimiento).split('T')[0] : '----/--/--'}
                  </td>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in">
          <div className="bg-white rounded-[32px] p-8 max-w-md w-full shadow-2xl border border-slate-100 animate-in zoom-in-95">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                {/* 👇 FIX APLICADO: Input date por defecto usa AAAA-MM-DD */}
                <div><label className="block text-xs font-black text-slate-400 uppercase mb-1">Nacimiento</label><input type="date" value={formData.fecha_nacimiento} onChange={e => setFormData({...formData, fecha_nacimiento: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 font-bold outline-none focus:border-blue-500 text-slate-600 tracking-wider" /></div>
                <div><label className="block text-xs font-black text-slate-400 uppercase mb-1">NIP (4 dígitos) *</label><input required type="text" maxLength="4" value={formData.nip} onChange={e => setFormData({...formData, nip: e.target.value.replace(/\D/g, '')})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 font-black tracking-[0.5em] text-center outline-none focus:border-blue-500 text-slate-700" placeholder="0000" /></div>
              </div>
              ...
              {/* 👇 NUEVA CARACTERÍSTICA: Edición directa de puntos solo habilitada al editar cliente */}
              {modoEdicion && (
                <div className="animate-in fade-in slide-in-from-bottom-2">
                  <label className="block text-xs font-black text-emerald-500 uppercase mb-1 flex items-center gap-1">
                    <Award size={14}/> Editar Puntos Monedero
                  </label>
                  <input type="number" min="0" value={formData.puntos} onChange={e => setFormData({...formData, puntos: Number(e.target.value) || 0})} className="w-full bg-emerald-50 border border-emerald-200 rounded-xl p-3 font-black outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 text-emerald-700 transition-all" />
                </div>
              )}
              ...
              <div className="flex gap-3 mt-6 pt-4 border-t border-slate-100">
                ...
                <button type="submit" disabled={isSubmitting} className="flex-[2] bg-blue-600 text-white py-3 rounded-xl font-black shadow-md hover:bg-blue-700 transition active:scale-95 disabled:opacity-50">
