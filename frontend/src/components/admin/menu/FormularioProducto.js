...
const FormularioProducto = ({
  ...
  useEffect(() => {
    if (productoEditando) {
      ...
      setNombre(p.nombre); setDescripcion(p.descripcion || ''); setEmoji(p.emoji || '🍽️'); setTiempoPreparacion(p.tiempo_preparacion || 15); setImagenBlob(null); 
  ...
  }, [productoEditando, clasificaciones, catalogoIngredientes, setCategoriaSelect]);
