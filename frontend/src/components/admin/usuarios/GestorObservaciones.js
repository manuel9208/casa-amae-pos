...
// Array para la cuadrícula (Lunes a Domingo)
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                            {empleadosVisibles.map(emp => {
                                // 1. Buscamos qué tareas le tocan a este empleado hoy según su Puesto (Rol)
                                ...
                                return (
                                    <div key={emp.id} className="bg-slate-50 p-5 rounded-3xl border border-slate-200 shadow-sm flex flex-col">
                                        <div className="flex justify-between items-start mb-4 border-b border-slate-200 pb-3">
                                            <div>
                                                <p className="font-black text-slate-800 text-lg leading-none">{emp.nombre.split(' ')[0]}</p>
                                                    ...
                                                    {observacionesBase.map(obs => {
                                                        ...
                                                        return (
                                                            <button 
                                                                ...
                                                                className={`w-full flex items-center gap-2 p-2 rounded-lg border text-left transition-all ${isChecked ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm' : 'bg-white text-slate-500 border-slate-200 hover:border-indigo-300'}`}
                                                            >
