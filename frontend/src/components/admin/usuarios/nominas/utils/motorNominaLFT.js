...
// 1. UTILIDADES Y CALCULADORAS BASE
export const calcularSueldoDiarioInteligente = (sueldoBase, frecuencia) => {
    const base = Number(sueldoBase) || 0;
    switch (frecuencia) {
        case 'Mensual': return (base / 4.333) / 7;
        case 'Quincenal': return (base / 2.166) / 7;
        case 'Semanal': return base / 7; 
...
export const calcularSueldoPorHora = (sueldoBase, frecuencia, plantilla) => {
    ...
    // 👇 LA MAGIA AUTOMÁTICA: Si sus días no suman un múltiplo de 8 (es decir, son asimétricos como los de Alberto de 14h y 8h), forzamos su cálculo por hora real para que no pierda dinero.
...
// 2. MÉTODOS DE PAGO SEPARADOS

const pagarEmpleadoSemanal = (horasReales, hrsProgramadasDia, sueldoDiarioFijo, edicionManualAdmin, sueldoPorHora) => {
    // 👇 FIX: Si el admin edita manualmente a Alberto (que es Semanal pero de horas asimétricas), se le paga la hora estricta.
...
export const procesarMotorNomina = (empleado, asistenciasRango, fechasRango, reglasGlobales, justificaciones = {}) => {
    ...
    const sueldoDiarioFijo = calcularSueldoDiarioInteligente(prestaciones.sueldo_base, tipoSueldo);
    const sueldoPorHora = calcularSueldoPorHora(prestaciones.sueldo_base, tipoSueldo, plantilla);

    // Detectamos si es un empleado "Asimétrico" como Alberto (que no trabaja jornadas parejas de 8 horas)
    let horasTotalesSemana = 0;
    Object.values(plantilla).forEach(dia => {
        if (dia.activo && dia.entrada && dia.salida) {
            const [hE, mE] = dia.entrada.split(':').map(Number);
    ...
    let metricas = { diasProgramados: 0, diasAsistidos: 0, faltasInjustificadas: 0, retardosEventos: 0, retardosMinutosGlobal: 0, diasDescansoLaborados: 0, domingosTrabajados: 0, diasInhabilesDeApoyo: 0, sueldoOrdinario: 0, horasProgramadasTotales: 0 };
    ...
    fechasRango.forEach(fecha => {
        ...
        if (configDia.activo) {
            ...
            if (asistenciaHoy || horasReales > 0) {
                ...
                // 👇 LA MAGIA: Si el sistema lo detectó como Asimétrico (Alberto), lo fuerza a pagarle sus horas por el valor exacto ($50 en su caso).
