...
const ProgramaLealtad = ({ configGlobal, setConfigGlobal, apiUrl, showAlert }) => {
  ...
  const guardarLealtad = async (e) => {
    ...
    // 👇 SOLUCIÓN DEFINITIVA: Enviamos EXCLUSIVAMENTE la información de lealtad.
    // Como el backend hace un merge con la base de datos, el resto de datos de tu local 
    // (como la limpieza y los horarios) quedarán intactos.
    formData.append('puntos_activos', configGlobal.puntos_activos === undefined ? true : configGlobal.puntos_activos);
    formData.append('puntos_canje_activo', configGlobal.puntos_canje_activo === undefined ? true : configGlobal.puntos_canje_activo);
    formData.append('puntos_porcentaje', configGlobal.puntos_porcentaje !== undefined ? configGlobal.puntos_porcentaje : 10);
    formData.append('puntos_valor_peso', configGlobal.puntos_valor_peso !== undefined ? configGlobal.puntos_valor_peso : 1.00);
