...
const GestorCupones = ({ apiUrl, showAlert, showConfirm }) => {
  ...
  return (
    <div className="bg-white p-6 md:p-8 rounded-[40px] shadow-sm border border-slate-200 mt-8 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-rose-400 to-rose-600"></div>
      <h3 className="text-2xl font-black mb-6 text-rose-600 flex items-center gap-2 border-b pb-4">
        <Ticket size={28}/> Cupones de Descuento
      </h3>  
      ...
      <form onSubmit={crearCupon} className="bg-rose-50/30 p-6 rounded-3xl border border-rose-100 mb-8 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          <div>
            ...
            <input required value={nuevoCupon.codigo} onChange={e => setNuevoCupon({...nuevoCupon, codigo: e.target.value.toUpperCase().replace(/\s+/g, '')})} className="w-full p-3 bg-white border border-rose-200 rounded-xl outline-none font-black text-slate-700 uppercase focus:border-rose-400 transition" placeholder="Ej. VERANO20" disabled={isSubmitting}/>
          </div>
          <div>
            <label className="block text-xs font-black text-rose-600 uppercase mb-1">Tipo de Descuento *</label>
            <select required value={nuevoCupon.tipo} onChange={e => setNuevoCupon({...nuevoCupon, tipo: e.target.value})} className="w-full p-3 bg-white border border-rose-200 rounded-xl outline-none font-bold text-slate-700 focus:border-rose-400 transition" disabled={isSubmitting}>
          ...
          <div>
            ...
            <input required type="number" step="0.01" min="0.1" value={nuevoCupon.valor} onChange={e => setNuevoCupon({...nuevoCupon, valor: e.target.value})} className="w-full p-3 bg-white border border-rose-200 rounded-xl outline-none font-bold text-slate-700 focus:border-rose-400 transition" placeholder="Ej. 15" disabled={isSubmitting}/>
          </div>
          <div>
            <label className="block text-xs font-black text-rose-600 uppercase mb-1">Límite Usos</label>
            <input type="number" min="1" value={nuevoCupon.limite_usos} onChange={e => setNuevoCupon({...nuevoCupon, limite_usos: e.target.value})} className="w-full p-3 bg-white border border-rose-200 rounded-xl outline-none font-medium text-slate-700 focus:border-rose-400 transition" placeholder="Ilimitado" disabled={isSubmitting}/>
          </div>
          <div>
            <label className="block text-xs font-black text-rose-600 uppercase mb-1">Vencimiento</label>
            <input type="date" value={nuevoCupon.fecha_vencimiento} onChange={e => setNuevoCupon({...nuevoCupon, fecha_vencimiento: e.target.value})} className="w-full p-3 bg-white border border-rose-200 rounded-xl outline-none font-medium text-slate-700 focus:border-rose-400 transition" disabled={isSubmitting}/>
          </div>
        </div>
