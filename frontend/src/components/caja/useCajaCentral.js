...
export const useCajaCentral = (user, onLogout, onGoToKiosco) => {
  ...
  const cerrarCajaYSalir = async () => { 
    localStorage.removeItem(`fondo_caja_${user?.id}_${hoyStr}`);
    onLogout(); 
  };
  ...
  const procesarPago = async (estadoRechazo = null, esPostPago = false, pagosMixtos = null) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setModalPago(null); setMontoRecibido(''); 
  ...
  const actualizarEstadoPedido = async (pedidoOId, nuevoEstado, extraData = {}) => {
    ...
    try {
      ...
      if (estadoSeguro === 'Entregado' && pedidoFull?.metodo_pago === 'Por Cobrar') {
         // Mantener "Por Cobrar" porque el cliente está comiendo (Cuenta Abierta)
    ...
    try {
        ...
        if (ped) {
          await fetch(`${apiUrl}/pedidos/${id}/estado`, { 
            method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ estado_preparacion: ped.estado_preparacion }) 
          });
        }
        await cargarDataDinamica();
    } catch(e) {}
  ...
  const confirmarAgregarExtra = async (pedidoOriginal, itemIndex, extraObj) => {
    ...
    try {
      ...
      await fetch(`${apiUrl}/pedidos/${pedidoOriginal.id}/estado`, { 
        method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ carrito: items, total: nuevoTotal, estado_preparacion: pedidoOriginal.estado_preparacion }) 
      });
  ...
  const registrarCompraRapida = async (payload) => {
    ...
    try {
         ...
         // 👇 FIX: Solo limpiamos variables internas, dejando el modal abierto
  ...
  const enviarRespuestaCocina = async (e) => {
    ...
    try {
      ...
      if (accionAlerta === 'cancelar') {
        ...
        if (itemAfectadoIdx !== null && itemAfectadoIdx !== '') { items.splice(itemAfectadoIdx, 1); payloadEstado.carrito = items; }
        ...
        if (itemAfectadoIdx !== null && itemAfectadoIdx !== '') {
          ...
          if (propuestaChef.toLowerCase().includes('sin')) items[itemAfectadoIdx].nombre = `${items[itemAfectadoIdx].nombre} (${propuestaChef})`;
          else if (propuestaChef && !propuestaChef.toLowerCase().includes('quitar') && !propuestaChef.toLowerCase().includes('cancelar')) items[itemAfectadoIdx].nombre = `${items[itemAfectadoIdx].nombre} (Sustitución: ${propuestaChef})`;
          payloadEstado.carrito = items;
        }
        textoRespuesta = 'CAJA RESPONDE: Propuesta aceptada. Proceder.';
      }

      await fetch(`${apiUrl}/pedidos/${modalResolver.id}/alerta`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ alerta_cocina: textoRespuesta }) });
      await fetch(`${apiUrl}/pedidos/${modalResolver.id}/estado`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payloadEstado) });
  ...
  const pendientesDePago = pedidos.filter(p => {
    ...
    if (p.estado_preparacion === 'Entregado' && !['Por Cobrar', 'Pendiente'].includes(p.metodo_pago)) return false;
  ...
  const pedidosEnReparto = pedidos.filter(p => p.tipo_consumo === 'Domicilio' && p.estado_preparacion === 'En Camino');
  ...
  return {
    ...
    apiUrl, cargarDataDinamica, fondoRepartidor, actualizarFondoRepartidor,
