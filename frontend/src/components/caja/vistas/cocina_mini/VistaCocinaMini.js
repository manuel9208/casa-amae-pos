...
const VistaCocinaMini = ({ user, pedidos, empleadosPOS, apiUrl, isSubmitting }) => {
  ...
  // 👇 FILTRO INTELIGENTE: Días de la semana
  const diasSemana = ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'];
  const diaHoy = diasSemana[new Date().getDay()];

  // 👇 FILTRO: Solo personal de cocina Y que tengan turno hoy según su horario
  const personalCocina = empleadosPOS.filter(emp => {
    // 1. Filtrar estritamente por rol de cocina
    const esEquipoCocina = ['cocina', 'ayudante_cocina'].includes(emp.rol);
    
    // 2. Verificamos si en su horario_semanal está marcado que trabaja HOY
  ...
  // Siempre agregamos al usuario logueado al principio de la lista por si necesita hacer una acción rápida
    ...
    <div className="space-y-6 animate-in slide-in-from-bottom-4">
      <div className="bg-white p-4 rounded-[24px] border border-slate-200 shadow-sm flex flex-col sm:flex-row gap-4 items-start sm:items-center">
        <div className="flex items-center gap-2">
          <div className="bg-orange-100 text-orange-600 p-2 rounded-xl"><ChefHat size={20}/></div>
          <p className="text-xs font-black uppercase text-slate-400 tracking-widest">Preparando:</p>
        </div>
