...
const TarjetaRepartidor = ({
  ...
  const [modalLiquidarTodo, setModalLiquidarTodo] = useState(false);  
    ...
    // 👇 FIX: Separamos deudas de Efectivo puro/Mixto vs Deudas de Transferencia
  ...
  // Función de Liquidación Masiva (Cierra ÚNICAMENTE lo que es efectivo puro)
  ...
  return (
    <>
      <div className="bg-white p-6 md:p-8 rounded-[36px] border border-slate-200 shadow-sm space-y-4 hover:shadow-md transition-shadow duration-300 animate-in slide-in-from-right-4 relative z-10">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-slate-50 p-4 md:p-5 rounded-2xl border border-slate-100 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Botón Maestro de Liquidación Total (Solo aparece si hay deuda de efectivo) */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 mt-4">
          {listaPedidos.map(p => {
            // 👇 FIX: Identificación precisa del origen de la deuda
            const esTransferencia = String(p.direccion_entrega || '').toUpperCase().includes('TRANSFERENCIA') || p.metodo_pago === 'Transferencia';
            ...
            // Extracción inteligente del nombre del cliente desde la dirección
            ...
            return (
              <div key={p.id} className={`p-5 rounded-2xl border flex flex-col justify-between transition-all duration-200 group ${estaEnRuta ? 'bg-slate-50 border-slate-200 opacity-90' : 'bg-slate-50/60 border-slate-200 hover:border-pink-300 hover:shadow-sm'}`}>
                <div className="flex justify-between items-start pb-3 border-b border-slate-100 mb-3">
                  ...
                  <span className={`text-[9px] font-black px-2 py-1 rounded-md uppercase tracking-widest shrink-0 ${
                    ...
                    {esDeudaEfectivo ? 'Efectivo (Cobrar)' : esDeudaTransferencia ? 'Transferencia (Validar)' : 'Pagado Online/Caja'}
                  </span>
                </div>  
                ...
                {esDeudaEfectivo ? (
                  estaEnRuta ? (
                    <button
                      onClick={() => setPedidoAConfirmar({ ...p, enRuta: true, tipoCobro: 'Efectivo' })}
                    ...
                    <button
                      onClick={() => setPedidoAConfirmar({ ...p, enRuta: false, tipoCobro: 'Efectivo' })}
                    ...
                    <div className="flex gap-2 w-full">
                        ...
                        <button
                            onClick={() => setPedidoAConfirmar({ ...p, enRuta: estaEnRuta, tipoCobro: 'Efectivo' })}
      ...
      {/* MODAL DE LIQUIDACIÓN TOTAL (SOLO EFECTIVO) */}
      {modalLiquidarTodo && (
      ...
      {/* MODAL DE LIQUIDACIÓN INDIVIDUAL (INTELIGENTE: TRANFERENCIA / EFECTIVO) */}
      {pedidoAConfirmar && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[40px] p-6 md:p-8 max-w-md w-full shadow-2xl flex flex-col items-center text-center animate-in zoom-in-95 duration-200 border border-slate-100">
            ...
            <p className="text-slate-500 font-medium mb-6">
              ¿Confirmas liquidar y registrar <strong className="text-slate-800 text-lg">${pedidoAConfirmar.total}</strong> en {pedidoAConfirmar.tipoCobro === 'Transferencia' ? 'TRANSFERENCIA' : 'EFECTIVO'} de la orden <strong className="text-slate-800">#{pedidoAConfirmar.numero_pedido}</strong>?
            </p>  
            ...
            <div className="flex w-full gap-3">
              ...
              <button
                onClick={() => {
                  // Ejecutamos la acción según la vía elegida por el cajero
                ...
                className={`flex-1 py-4 text-white font-black rounded-2xl shadow-lg transition-all active:scale-95 flex justify-center items-center gap-2 ${
                    pedidoAConfirmar.tipoCobro === 'Transferencia' ? 'bg-indigo-500 hover:bg-indigo-600 shadow-indigo-500/30' : 'bg-emerald-500 hover:bg-emerald-600 shadow-emerald-500/30'
                }`}
              >
                <CheckCircle2 size={20} />
                Confirmar
