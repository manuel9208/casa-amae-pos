...
const ModalAsistencia = ({ modalAsistencia, setModalAsistencia, apiUrl, setAlertaCaja }) => {
  ...
  useEffect(() => {
      ...
      if (pinInput.length === 4 && !isSubmitting) {
        ...
        try {
          ...
          if (res.ok) {
            ...
            setErrorAnim(true);
            // 👇 SOLUCIÓN AL BLOQUEO: Liberar el isSubmitting hasta que el PIN se haya borrado
            setTimeout(() => { 
              setErrorAnim(false); 
              setPinInput(''); 
              setIsSubmitting(false); 
    ...
    procesarChecada();
  }, [pinInput, isSubmitting, modalAsistencia, apiUrl, setAlertaCaja, setModalAsistencia]);  
