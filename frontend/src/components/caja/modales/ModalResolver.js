...
const ModalResolver = ({
  ...
  ingredienteReemplazo, setIngredienteReemplazo, enviarRespuestaCocina, catalogoIngredientes, isSubmitting
}) => {
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <form onSubmit={enviarRespuestaCocina} className="bg-white p-8 rounded-[40px] shadow-2xl border border-slate-200 w-full max-w-xl">
        ...
        <div className="bg-red-50 p-4 rounded-2xl mb-6 border border-red-100">
          ...
          <p className="text-lg font-black text-red-700">{modalResolver.alerta_cocina.replace(/\[IDX:\d+\]\s*/g, '')}</p>
        </div>
