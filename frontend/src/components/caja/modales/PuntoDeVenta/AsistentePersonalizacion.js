...
const AsistentePersonalizacion = ({
  ...
  extrasSeleccionados, setExtrasSeleccionados,
  notaProducto, setNotaProducto,
  ...
  // Extraemos la lógica pesada de armar el platillo final
  const handleAñadir = () => {
    const extrasFinales = [];
    if (opcionSeleccionada && opcionSeleccionada.precioExtra > 0) {
      extrasFinales.push({ nombre: opcionSeleccionada.nombre, precioExtra: opcionSeleccionada.precioExtra, tipo: 'variacion' });
    }
    if (saborSeleccionado) {
      extrasFinales.push({ nombre: saborSeleccionado.nombre, precioExtra: saborSeleccionado.precioExtra, tipo: 'variacion' });
    }
    ...
    if (notaProducto.trim() !== '') {
      extrasFinales.push({ nombre: `📝 ${notaProducto}`, precioExtra: 0, tipo: 'nota' });
    }
    ...
    const nuevoItem = {
      ...
      // 👇 INYECCIÓN QUIRÚRGICA: Estos datos evitan que el backend falle al cobrar
      ...
      <div className="bg-slate-50 rounded-[32px] md:rounded-[40px] shadow-2xl w-full max-w-xl overflow-hidden flex flex-col max-h-[90vh] border border-slate-100">
        ...
        <div className="p-6 md:p-8 bg-white border-t border-slate-200 shrink-0">
          ...
          <div className="flex gap-3 md:gap-4">
            <button onClick={resetWizard} className="flex-1 py-4 md:py-5 bg-slate-50 text-slate-600 font-black rounded-xl md:rounded-2xl hover:bg-slate-200 transition border border-slate-200 text-sm md:text-base">
              Cancelar
            </button>
